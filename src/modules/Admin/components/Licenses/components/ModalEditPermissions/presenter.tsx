      ...
      content: (
        <div {...classes("tab-content")}>
          <div {...classes("pending-voc")}>
            ...
            <div {...classes("pending-date-picker")}>Expired date</div>
          ...
          {pendingVocabularies.map((voc: Vocabulary) => (
            <div {...classes("pending-voc")}>
              ...
              <div {...classes("pending-date-picker")}>
                <Field component={DatepickerControler} name={`expiredDates[${voc.licenseId}]`} />
              </div>
              <div {...classes("pending-button")}>
                <Field component={VocRadioButton} name={`pendingVocabs[${voc.licenseId}]`} options={{ value: true }} />
              </div>
              <div {...classes("pending-button")}>
                <Field component={VocRadioButton} name={`pendingVocabs[${voc.licenseId}]`} options={{ value: false }} />
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div {...classes()}>
      <Modal modal={modal} title={`Edit permissions for user ${user}`} mods={["no-padding"]}>
        <form onSubmit={handleSubmit(doSubmit)} {...props}>
          {pendingVocabularies.length > 0 ? <TabbedPane sections={sections} /> : sections[0].content}
          <div {...classes("submit-button-wrapper")}>
            <Button {...classes("submit-button")} type="submit" mods={["submit", "rounded"]}>
              Save
            </Button>
          </div>
        </form>
